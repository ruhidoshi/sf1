def clook_disk_scheduling(requests, head_start, direction="left"):
    ...
    while requests_copy:
        ...
        # If there are remaining requests, move to the beginning of the disk
        if requests_copy:
            total_head_movement += abs(current_head_position - min(requests_copy))
            current_head_position = min(requests_copy)
...
if __name__ == "__main__":
    ...
    # Call the clook_disk_scheduling function with the specified requests, head position, and direction
    total_movement = clook_disk_scheduling(requests, initial_head_position, direction="left")
